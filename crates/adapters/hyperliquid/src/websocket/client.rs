...
/// Bound on the cloid -> client_order_id resolution cache.
///
/// FIFO eviction matches the rest of the adapter's cleanup-on-overflow model
/// (see `FifoCache` uses in `WsDispatchState` and `FeedHandler`) so the cache
/// self-evicts even when `remove_cloid_mapping` is bypassed (for example, the
/// drain path in `dispatch::handle_accepted` for a buffered cancel-replace
/// fill, see GH-3972).
pub(super) const CLOID_CACHE_CAPACITY: usize = 10_000;

/// Shared cloid -> client_order_id cache backing
/// [`HyperliquidWebSocketClient::cache_cloid_mapping`] and the WS handler
/// resolution path.
...
impl HyperliquidWebSocketClient {
    ...
    /// The cache is FIFO-bounded so missed removals self-evict on overflow rather than
    /// leaking forever (relevant to the cancel-replace drain path; see GH-3972).
    #[allow(
        clippy::missing_panics_doc,
        reason = "cloid cache mutex poisoning is not expected"
    )]
