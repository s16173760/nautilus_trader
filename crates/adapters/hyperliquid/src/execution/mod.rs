...
fn handle_execution_report(
    ...
    match report {
        ...
        ExecutionReport::Fill(fill_report) => {
            ...
            // mapping now that the fill has landed. Skip when the fill was
            // buffered for a cancel-replace race (GH-3972): the replacement
            // `ACCEPTED(new_voi)` still has to resolve the cloid before the
            // buffer can be drained, so evicting now would leave the buffered
            // fill stranded.
            if let Some(id) = client_order_id
